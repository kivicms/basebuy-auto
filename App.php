...
class App {
    ...
    private function updateModifications() {
        \helpers\Console::log('Проверяем таблицу car_modification');
        foreach ($this->types as $type) {
            if ( $this->basebuyAutoApi->modificationGetDateUpdate( $type ) > $this->lastDatesUpdate['modification']){
    
                if ($type > 1) {
                    return;
                }
                echo "Тип: " . $type . "\n";
                $this->downloadedFilePath = $this->basebuyAutoApi->modificationGetAll( $type );
    
    
                ORM::configure('id_column', 'id_car_modification');
    
                $fileData = $this->parseFile();
    
                foreach ($fileData as $fd) {
    
                    $rec = ORM::for_table('car_modification')->where('id_car_modification', $fd[0])->find_one();
    
                    if (! $rec) {
                        $rec = ORM::for_table('car_modification')->create();
                        $rec->id_car_modification       = $fd[0];
                        $rec->id_car_serie              = $fd[1];
                        $rec->id_car_model              = $fd[2];
                        $rec->name                      = $fd[3];
                        $rec->start_profuction_year     = $fd[4];
                        $rec->end_production_year       = $fd[5];
                        $rec->date_create               = $fd[6];
                        $rec->date_update               = $fd[7];
                        $rec->id_car_type               = $type;
                        $rec->manual_update             = 0;
                        $rec->save();
                    } else {
                        if ($rec->manual_update) {
                            Console::log('Таблица car_modification:', 'normal', true);
                            $this->consoleArray('Старая запись', $fd, 'green');
                            $this->consoleArray('Новая запись', [$rec->id_car_modification, $rec->name], 'red');
